...
let lastHistoryAt=0,lastHistoryKey='';
function docType(){return clean(cfg.documentType)||(global.PCPRCore&&typeof global.PCPRCore.moduleName==='function'?global.PCPRCore.moduleName():clean(document.title)||'Documento')}
function currentAuthority(){
  try{
    const c=global.PCPRConfig&&typeof global.PCPRConfig.get==='function'?global.PCPRConfig.get():{};
    const candidates=['#autoridade','#authority','#delegado','#responsavel','#signatario','select[name*=autoridade]','select[name*=delegado]'];
    for(const sel of candidates){const el=document.querySelector(sel);if(el&&clean(el.value))return clean(el.value)}
    if(cloudSelectors.length){
      const a=global.PCPRConfig&&typeof global.PCPRConfig.authority==='function'?global.PCPRConfig.authority(c):((c.autoridades||[])[0]||{});
      return clean(a&&a.nome);
    }
    return '';
  }catch(_){return ''}
}
function historyAdd(action,filename,format,result,extra){
  try{
    const h=global.PCPRCore&&global.PCPRCore.history;if(!h||typeof h.add!=='function')return;
    const key=[action,filename,format].join('|'),now=Date.now();
    if(key===lastHistoryKey&&now-lastHistoryAt<1200)return;
    lastHistoryKey=key;lastHistoryAt=now;
    h.add({tipo:docType(),nomeDoc:getBaseName(),arquivo:clean(filename),autoridade:currentAuthority(),acao:action,formato:clean(format),resultado:clean(result)||'Sucesso',...(extra||{})});
  }catch(_){}
}
  ...
  proto.click=function(){
    try{
      if(armed&&Date.now()<armed.until&&this.download){
        ...
        historyAdd('GERADO',this.download,ext.toUpperCase(),'Sucesso');
...
function patchPrint(){
  ...
  function wrappedPrint(){
    ...
    historyAdd('GERADO',base+'.pdf','PDF','Impressão/Salvar PDF acionado');
    return nativePrint();
  }
...
function patchJsPdf(){
  ...
  function wrappedSave(name){
    ...
    const result=original.call(this,outName,...Array.prototype.slice.call(arguments,1));historyAdd('GERADO',outName,ext.toUpperCase(),'Sucesso');setTimeout(()=>{document.title=originalTitle},0);return result;
  }
...
function init(){
  ...
  global.PCPRDocumento={VERSION,getBaseName,fileName,requireName,safeBase,historyAdd,docType,currentAuthority};global.PCPRDocName=global.PCPRDocumento;
}
